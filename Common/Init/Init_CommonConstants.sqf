...
CTI_TOWNS_OCCUPATION_DETECTION_RANGE_AIR = 60; //--- Determine how high a threat is considered aerial
...
//--- Base: Parameters
with missionNamespace do {
	if (isNil 'CTI_BASE_AREA_MAX') then {CTI_BASE_AREA_MAX=if !(MADE_FOR_STRATIS) then {250} else {100};};
...
with missionNamespace do {
	...
	if (isNil 'CTI_PLAYER_REEQUIP') then {CTI_PLAYER_REEQUIP = 2}; //Done
	...
	if (isNil 'CTI_GAMEPLAY_3P') then {CTI_GAMEPLAY_3P = 0}; //Done
	...
	if (isNil 'CTI_TOWNS_SHOPS') then {CTI_TOWNS_SHOPS = 1}; //Done



	if (isNil "CTI_SM_BASEP") then {CTI_SM_BASEP=0};
