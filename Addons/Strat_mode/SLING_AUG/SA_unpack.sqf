...
if (_target == ((CTI_P_SideJoined) call CTI_CO_FNC_GetSideHQ)) then {
	...
	if (CTI_BASE_NOOBPROTECTION == 1) then {
		_hq addEventHandler ["handleDamage", format["[_this select 2, _this select 3, %1] call CTI_CO_FNC_OnHQHandleDamage", ((_side) call CTI_CO_FNC_GetSideID )]]; //--- You want that on public
	};
	((_side) call CTI_CO_FNC_GetSideLogic) setVariable ["cti_hq", _hq, true];
	[["CLIENT", _side], "Client_givehqlock", []] call CTI_CO_FNC_NetSend;
