...
if (_target == ((CTI_P_SideJoined) call CTI_CO_FNC_GetSideHQ)) then {
	...
	_packobject setVariable ["cti_gc_noremove", true]; //--- HQ wreck cannot be removed nor salvaged
	_packobject setVariable ["cti_ai_prohib", true]; //--- HQ may not be used by AI as a commandable vehicle
	...
	if (CTI_BASE_NOOBPROTECTION == 1) then {
		_packobject addEventHandler ["handleDamage", format["[_this select 2, _this select 3, %1] call CTI_CO_FNC_OnHQHandleDamage",( (_side) call CTI_CO_FNC_GetSideID)]]; //--- You want that on public
	};
	...
	sleep 2;
	["SERVER", "SA_LOOP",_target] call CTI_CO_FNC_NetSend;
};
