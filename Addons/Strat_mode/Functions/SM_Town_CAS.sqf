SM_TCAS_MAINLOOP=compile '
	...
	[_group,_v,_town] spawn {
		private ["_wp2","_v","_group","_town"];
		_group=_this select 0;
		_v=_this select 1;
		_town =_this select 2;
		while {!CTI_GameOver && alive _v && !isnull _group && count (units _group)>0 } do {
			sleep 10;
			if ( (currentWaypoint _group )> (count (waypoints _group) -5) ) then {
				_wp2=_group addWaypoint [[[[(getPos _town),600]]] call BIS_fnc_randomPos, 0];
