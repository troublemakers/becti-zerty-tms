...
switch (_action) do {
	case "onLoad": {
		if (_target iskindof "Man") then {((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510102)) ctrlsettext (format ["%1", name _target])} else {((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510102)) ctrlsettext (getText (configFile >> "CfgVehicles" >> typeof _target >> "displayname"))};
		_offset=0;
		for "_i" from 0 to _max_ctrl /* step +1 */ do {
			switch (_i) do {
			    case 1: {// CTI_Icon_tablet // ok
					...
					((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
		    		_offset=_offset+1;
			    };
			    case 2: {// CTI_Icon_Lock // ok
			    ...
			    case 3: { // CTI_Icon_Net // ok
					if ( alive _target && (missionNamespace getVariable "CTI_EW_ANET" == 1) && ( _target iskindof "Car" || _target iskindof "Tank" || _target iskindof "Air"|| _target iskindof "Ship") && !(_target isKindOf "I_G_Offroad_01_F")) then  {
						((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
				...
				case 4: { // CTI_Icon_inventory
					...
					if (alive _target && count _possible >0 && locked _target <2) then  {

						if (_target in _possible) then {((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0,0,1,1];} else {
							if (count _possible > 0) then {((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0.3,0.3,0.3,1];};
						};
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
			    		_offset=_offset+1;
			    	} else {
				...
				case 5: { // CTI_Icon_Def // ok
			    	if (vehicle player == player && alive _target&& (_target == ((CTI_P_sidejoined) call CTI_CO_FNC_GetSideHQ) || (missionNamespace getVariable [format ["%1", typeOf _target],["","","","","","","",""]]) select 7 == "service-repairtruck"  || ((missionNamespace getVariable [format ["CTI_%1_%2", CTI_P_SideJoined, typeOf _target],[[""]]]) select 0) select 0  == CTI_REPAIR)) then  {
				...
				case 6: { // CTI_Icon_Rep //ok
			    	if (vehicle player == player &&( _target iskindof "Car" || _target iskindof "Tank" || _target iskindof "Air"|| _target iskindof "Ship" || _target iskindof "Wheeled_APC_F"|| _target iskindof "Truck_F") && getDammage _target > 0 && alive _target) then  {
			    		if  ({_x == "Toolkit"} count (backpackItems player) >0) then {((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0,0,1,1]; } else {((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0.3,0.3,0.3,1];};
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
			    		_offset=_offset+1;
			    	} else {
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y+5,_base_w,_base_h];
			    ...
			    case 13: { // CTI_Icon_fact // ok
			    	if (_target in (CTI_P_SideJoined call CTI_CO_FNC_GetSideStructures) && alive _target && !isnil {missionNamespace getVariable format ["CTI_%1_%2", CTI_P_SideJoined, typeOf _target]}) then  {
			    		...
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510102)) ctrlsettext (format ["%1", ((_var select 0) select 2)]);
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0,0,1,1];
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
			    		_offset=_offset+1;
			    	} else {
			    ...
			    case 21: { // CTI_Icon_build
			    	if (vehicle player == player && _target in (CTI_P_SideLogic getVariable ["cti_structures_wip",[]])) then  {
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0,0,1,1];
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
			    		_offset=_offset+1;
			    	} else {
			    ...
			    case 22: { // CTI_Icon_srep
			    	if (vehicle player == player && _target in (CTI_P_SideLogic getVariable ["cti_structures",[]])&&( getDammage _target >0 ||  _target getVariable ["cti_altdmg",-1] >0) ) then  {
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0,0,1,1];
			    		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetPosition [_base_x+(_offset*_base_w),_base_y,_base_w,_base_h];
			    		_offset=_offset+1;
			    	} else {
			    ...
			    case 25: {// CTI_Icon_Pilot
			    	...
			    	if (! isnull _target && alive _target && vehicle player == player &&(_target isKindOf "B_Heli_Transport_03_base_F" || _target isKindOf "Heli_Transport_04_base_F" )) then  {
			    		if (!((304400  in (getdlcs 1)) || (288520  in (getdlcs 1)))&& locked _target < 1 && isNull (driver _target))  then {
			    			((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0,0,1,1];
			    		} else {
			    			((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (511000+_i)) ctrlSetTextColor [0.3,0.3,0.3,1];
			    		};
		...
		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510100)) ctrlSetPosition [_base_x- _base_w*0.20,_base_y- _base_h*0.20-SafeZoneH * 0.03,0,_base_h*1.4 +SafeZoneH * 0.03];
		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510101)) ctrlSetPosition [_base_x- _base_w*0.10,_base_y- _base_h*0.10,0,_base_h*1.2 ];
		...
		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510100)) ctrlSetPosition [_base_x- _base_w*0.2,_base_y- _base_h*0.2-SafeZoneH * 0.03,((_offset+0.4)*_base_w) max (SafeZoneW*0.15),_base_h*1.4+SafeZoneH * 0.03 ];
		...
		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510102)) ctrlSetPosition [_base_x,_base_y- _base_h*0.20-SafeZoneH * 0.03,SafeZoneW*0.15,SafeZoneH * 0.03];
		...
		((uiNamespace getVariable "cti_dialog_ui_interractions") displayCtrl (510102)) ctrlcommit 0;
	};
	case "OnLock": {
	...
	case "OnPilot": {
		closedialog 0;
		if  (isNull (driver _target)) then {player moveInDriver _target};
	};
};